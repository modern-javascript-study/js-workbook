...
let User = class MyClass{
    sayHi()[
        consolo.log(MyClass); // MyClass라는 이름은 오직 클래스 안에서만 사용할 수 있다. 
    ]
};
...
왜 구지 이렇게 하는거지?? => 객체 선언안하고 바로 가능해서??
...
이렇게 this의 컨텍스트를 알 수 없게 되면 문제는 '잃어버린 this'라고 한다. 

문제를 해결하기 위해 두 개의 방법을 사용할 수 있는데 함수 바인딩에서 이 방법에 대해 살표볼 수 있다. 
